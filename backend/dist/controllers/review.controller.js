...
export const createOrUpdateReview = asyncHandler(async (req, res) => {
    ...
    const review = await ReviewModel.findOneAndUpdate({ user: req.auth.sub, product: body.productId }, {
        $set: {
            rating: body.rating,
            title: body.title ?? "",
    ...
    const stats = agg[0];
    await ProductModel.updateOne({ _id: product._id }, { $set: { ratingAvg: stats?.avg ?? 0, ratingCount: stats?.count ?? 0 } });
